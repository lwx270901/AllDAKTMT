...
string generateRandomString() {
    ...
    string result = "";
    for (int i = 0; i < 6; i++) {
        int randomDigit = rand() % 10;  // Generate a random digit between 0 and 9
        result += to_string(randomDigit);  // Add the random digit to the string
    }
    return result;
}
...
void FaceNetClassifier::forward(cv::Mat frame, std::vector<struct Bbox> outputBbox) {
    ...
    for(int i = 0; i < m_croppedFaces.size(); i++) {
        ...
        //detect fake face
        float x1 = m_croppedFaces[i].x1;
        float y1 = m_croppedFaces[i].y1;
        float x2 = m_croppedFaces[i].x2;
        float y2 = m_croppedFaces[i].y2;
        struct LiveFaceBox LiveBox = {x1, y1, x2, y2};
        auto prob = Live.Detect(frame, LiveBox);
        if(prob <= 0.6 )
        {
            m_croppedFaces[i].isFake = true;
        }
        //end detect
...
void FaceNetClassifier::featureMatching(cv::Mat &image, MqttPublisherThread& publisherThread, std::vector<struct Bbox> outputBbox) {
    ...
    for(int i = 0; i < (m_embeddings.size()/128); i++) {
        ...
        if(m_croppedFaces[i].isFake == true)
        {
            cv::putText(image, "Fake", cv::Point(m_croppedFaces[i].y1+2, m_croppedFaces[i].x2-3),
                    cv::FONT_HERSHEY_DUPLEX, 0.1 + 2*fontScaler,  cv::Scalar(0,0,255,255), 1);
                    if(!fake_flag) {
                        addNewAbnomal(image, outputBbox, "-fake");
                        fake_flag = true;
                    }
        }

        else if (minDistance <= m_knownPersonThresh) {
            cv::putText(image, m_knownFaces[winner].className, cv::Point(m_croppedFaces[i].y1+2, m_croppedFaces[i].x2-3),
                    cv::FONT_HERSHEY_DUPLEX, 0.1 + 2*fontScaler,  cv::Scalar(0,0,255,255), 1);
                    std::string message = m_knownFaces[winner].className;
                    publisherThread.publish(message);
                    fake_flag = false;
        }
            ...
            cv::putText(image, "New Person", cv::Point(m_croppedFaces[i].y1+2, m_croppedFaces[i].x2-3),
                    cv::FONT_HERSHEY_DUPLEX, 0.1 + 2*fontScaler ,  cv::Scalar(0,0,255,255), 1);
                    addNewAbnomal(image, outputBbox, "-abnomal");
        }
    }
}
...
void FaceNetClassifier::addNewAbnomal(cv::Mat &image, std::vector<struct Bbox> outputBbox, string event){
	string newName = generateRandomString();
	// newName.append("-abnomal");
    newName.append(event);
	forwardAddFace(image, outputBbox, newName);
	string filePath = "../abnomal_images/";
	filePath.append(newName);
	filePath.append(".jpg");
	cv::imwrite(filePath, image);
}
